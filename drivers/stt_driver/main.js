...
const {mqttlib:mqtt} = require('./mqttlib');
const app = express();
...
const SERVER_PORT = 9108; // websocket server port
...
function endAudioStream(callback) {
	...
	if (results) {
		if (callback) {
			...
			mqtt.send("/speech", JSON.stringify({data:results.text, layer:"", ts:Date.now()}));
			callback(results);
...
io.on('connection', function(socket) {
	...
	socket.onmessage = function(event) {
		processAudioStream(event.data, (results) => {
